...
#
# 注意：本脚本不会自动改你的 docker-compose.yml，因为无法得知你的 service 名和缩进格式
...
echo -n "请输入 MariaDB root 密码（回车跳过=root 用户无密码，2023+ 版本应该都是空密码）: "
read -s DB_PASSWORD
echo
...
echo -e "${YELLOW}注意：本脚本不会自动改你的 SYZOJ 源代码${NC}"
