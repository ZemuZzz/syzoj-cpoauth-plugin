...
对应密钥。**只在服务端使用，不要写进前端代码或 view 模板**。
...
**这个地址必须跟 cpoauth.com 上注册的"应用回调 URL"完全一致**（包括协议、域名、路径、末尾斜杠），否则请求会被拒绝。
...
> /userinfo 返回的字段叫 `linked_accounts`（array），不是 `cp_summary`。详见 TROUBLESHOOTING.md。

### `SYZOJ_WEB_CPOAUTH_BASE_URL` （默认值即可）

如果你的 cpoauth 部署在自定义域名（一般是开发 / 测试环境会用到），可以改这个。**生产环境保持默认。**
