...
- 你已经在 [cpoauth.com](https://cpoauth.com) 注册了应用并拿到 `client_id` / `client_secret`
- 你的 SYZOJ 站点有公网域名 + HTTPS(回调地址必须 https)
...
2. 创建新应用,填写:
   ...
   - **Scope**: 勾选 `openid`、`profile`、`cp:linked` 三项
...
- 打印 docker-compose.yml 需要添加的 volumes 和 environment 段(供你 copy-paste)
...
> **注意**: 必须 `--force-recreate`,光 `restart` 不会重新加载新的 volume 挂载。
