...
检查 docker-compose.yml 的 `environment` 段,或 `docker exec <web> env | grep CPOAUTH` 看变量是否注入容器。
...
- 必须**完全一致**。包括协议（http/https）、域名、路径（`/auth/cpoauth/callback`）、**末尾斜杠**
...
严格用 `openid profile cp:linked`（带冒号）。检查 docker exec 看环境变量实际值。
...
严格按 [user_module_integration.md](../integration/user_module_integration.md) 改 `modules/user.js`。

### 8. CSP 违规，浏览器 console 报 `Refused to connect to cpoauth.com`

**原因**: nginx 加了 Content-Security-Policy 但没把 cpoauth.com 放白名单。

参考 [examples/nginx-csp.example](../examples/nginx-csp.example)，在 `connect-src` 里加 `https://cpoauth.com`。注意 nginx `if` 块的 add_header 可能会**覆盖**外层。

### 9. POST `/auth/cpoauth/create` 报 `用户名已存在`

让用户在选择页手动改用户名（field 是允许编辑的）。
