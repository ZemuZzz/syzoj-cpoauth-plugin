...
直接登录到那个用户。如果当前已登录另一个 SYZOJ 账号 → 报错(防止冒用)。

### 分支 2: 未绑定 + 当前已登录 SYZOJ

把这个 CP OAuth 绑定到当前登录的 SYZOJ 账号。常见场景:用户从个人主页点"绑定 CP OAuth"按钮。

### 分支 3: 未绑定 + 未登录

跳转到 `/auth/cpoauth/choose` 选择页,让用户选:
- **创建新账号** → 用 CP OAuth 资料创建一个新 SYZOJ 用户(用户名/邮箱/密码自己填,密码用于站内登录)
