...
> `_cpoauth_profile_card.ejs` 内部本身就用 `<div class="row">...</div>` 结构，所以直接插在外层 grid 的 row 之间就行，无需额外嵌套。
...
卡片的紫色渐变色 `linear-gradient(90deg, #5c2db8, #7c4dd8)` 直接写在 partial 里。如想换颜色，可编辑 `plugin/views/_cpoauth_profile_card.ejs`。
