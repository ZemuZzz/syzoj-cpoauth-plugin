...
> 注意:include 路径**不能带 `.ejs` 后缀**,SYZOJ 的 EJS 配置如此。

## 完整示例

修改前(原生 SYZOJ `views/login.ejs` 关键片段):
...
- 按钮**只在插件正确配置时显示**(检测 `syzoj.cpoauth_enabled`)。如果环境变量没设置,按钮不会出现,不会破坏现有登录页
